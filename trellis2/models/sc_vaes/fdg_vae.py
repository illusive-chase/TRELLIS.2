...
from .sparse_unet_vae import SparseUnetVaeDecoder, SparseUnetVaeEncoder
...
class FlexiDualGridVaeDecoder(SparseUnetVaeDecoder):
    ...
    def forward(self, x: sp.SparseTensor, gt_intersected: sp.SparseTensor = None, **kwargs):
        ...
        if self.training:
            ...
            quad_lerp = h.replace(F.softplus(h.feats[..., 6:7]))
            mesh = [Mesh(*flexible_dual_grid_to_mesh(
                local_h.coords[:, 1:], v.feats, i.feats, q.feats,
                aabb=[[-0.5, -0.5, -0.5], [0.5, 0.5, 0.5]],
                grid_size=self.resolution,
                train=False
            )) for local_h, v, i, q in zip(h, vertices, intersected, quad_lerp)]
            out_list[0] = mesh
